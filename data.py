...
class Dataset:
    """
    Class handling tf_dataset creation for a segmentation dataset from two
    directories, containing 2D images and corresponding labels as png.

    All images in image_dir have to have a label with the same filename in
    label_dir. Images which do not have a corresponding label file in
    label_dir are assumed to be test images.
    This class can split these images in train/val folds and optionally
    normalise them. The way categories are calculated from label images can
    be changed in the function _load_label_images.

    Usage: An example on how to use this class can be found in experiment.py
    """
    ...
    def _load_labels(self, purpose=None, cross_validation=False, fold=None,
        ...
        for path in filtered_df.img_name:
            ...
            # assume three classes: pixel == 0, pixel == 2, and everything else
...
class DataAugmentor:
    ...
    Usage: An example on how to use this class can be found in experiment.py
    """
    def __init__(self, flip_left_right=0.5, flip_up_down=0.5,
